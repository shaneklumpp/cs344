/*Shane Klumpp Project 4; otp_dec.c file. Uses the client.c file as a starting place.
...
#define DECIPHER_TEXT_MAX_SIZE	70005
...
int main(int argc, char *argv[])
{
	...
	char* keyText = NULL;
	char* cipherTextFile = argv[1];	//cipherText File
	char* keyTextFile = argv[2];	//keyText File
    ssize_t cipherTextTotalLength; //total length of cipher
    ssize_t keyTextTotalLength; //total length of key
	int cipherTextLen; //actual length
	int keyTextLen; //actual length
	
	cipherTextTotalLength = readFile(&cipherText, cipherTextFile); //read from cipherText file into cipherText and get size
	cipherTextLen = strlen(cipherText); //get actual length
	//printf("String from cipherTextFile\n");
	//printf("%s",cipherText);
	//printf("Length of cipherText ssize_t: %zu\n", cipherTextTotalLength);
	//printf("Length of cipherText int: %d\n",cipherTextLen);
	//printBuffer(cipherText, cipherTextTotalLength);
	
	//null terminator in place of \n char
	cipherText[strcspn(cipherText, "\n")] = '\0'; //\n -> \0
	//printBuffer(cipherText, cipherTextTotalLength);
	cipherTextLen = strlen(cipherText);
	//printf("String from cipherTextFile after newline to null terminator (newline added manually for readability)\n");
	//printf("%s\n",cipherText);
	//printf("Length of cipherText int after newline to null terminator: %d\n\n",cipherTextLen);
	...
	if(keyTextLen < cipherTextLen){ //check if keylength is shorter than cipherText length
	...
	int badCharsResult;
	badCharsResult = checkBadChars(cipherText, cipherTextLen); 
	if(badCharsResult == -1){
		fprintf(stderr, "Bad Characters in %s\n", cipherTextFile); //print error message
		exit(1); //exit 1 error
	}
	...
	//1) send "decoder" messsage to server. Exit with status 2 if doesn't recieve correct signal back
	//2) send cipherText to otp_enc_d
	...
	****************************************************************************************************/
	...
	/********
	 * 1) Confirmation decoder message
	********/
	//set up decoder message signaling server that it is an otp_enc client
	char confirmMessage[8] = "decoder";
	...
	if(returnStatus == 0){ //it did so terminate
		fprintf(stderr, "otp_dec tried to connect to otp_enc_d on port %d; illegal operation terminating\n", portNumber); //print error message
	 ...
	 * 2) Send cipherText to otp_enc_d
	********************/
	int convertedcipherTextSize = htonl(cipherTextLen); //convert cipherTextLen into sendable data
	charsWritten = send(socketFD, &convertedcipherTextSize, sizeof(convertedcipherTextSize), 0);
	if (charsWritten < 0) error("CLIENT: ERROR writing to socket");
	if (charsWritten < sizeof(convertedcipherTextSize)) printf("CLIENT: WARNING: Not all data written to socket!\n");
	
	//send cipherText to server
	size_t cipherTextTry;
	do{
		cipherTextTry = 0;
		cipherTextTry = writeToFD(socketFD, cipherText, strlen(cipherText));
		//printf("cipherTextTry = %zu\n", cipherTextTry);
	} while ((cipherTextTry != (size_t) cipherTextLen)); //send until full length is sent
	//at this point we know the correct amount of information was sent to server so we know it has cipherText
	...
	****************/
	//send keysize
	int convertedKeyTextSize = htonl(keyTextLen); //convert cipherTextLen into sendable data
	...
	******************/
	//recieve decipherText. Create decipherTextRecv with DECIPHER_TEXT_MAX_SIZE to handle maximum c string size
	char decipherTextRecv[DECIPHER_TEXT_MAX_SIZE]; //allocate 70000 byte buffer for maximum decipherText size
	memset(decipherTextRecv, '\0', DECIPHER_TEXT_MAX_SIZE); //null terminator out
	...
	if(retval == -1){
		...
		//printf("CLIENT: select() returned. decipherText can be read now\n");
		//printf("CLIENT: checking FD int socketFD: %d\n", socketFD);
		
		//recieve cipher text into decipherTextRecv
		size_t decipherTextTry;
		//int counter = 0;
		do{
			decipherTextTry = 0;
			decipherTextTry = readFromFD(socketFD, decipherTextRecv, strlen(cipherText)); //should be same size as cipherText not decipherText
			//fprintf(stderr, "decipherTextTry = %zu\n", decipherTextTry);
			
			//counter++;
			//if(counter == 2){ break;}
			
		} while(decipherTextTry != (size_t) cipherTextLen); //read until full length is read of expected text
		//fprintf(stderr, "CLIENT: Got a message. Not going to print now because I am just testing\n");
		//printf("CLIENT: decipherTextTry: %zu, expected: %d\n", decipherTextTry, cipherTextLen); //compare decipherTextTry to cipherTextLen
	...
	//fprintf(stderr, "CLIENT: size of decipherTextRecv to stdout: %zu\n", strlen(decipherTextRecv));
	//output decipherText to stdout
	
	/*************************
	 * FINAL PRINT TO STD OUT ALWAYS KEEP THIS ACTIVE!!!
	*************************/
	fprintf(stdout, "%s\n", decipherTextRecv); //output decipherTextRecv with a \n character
	...
	*************************/
	...
	//printf("CLIENT: I received this from the server: \"%s\"\n", decipherTextRecv);
	//printf("Server: Length of decipherText Message: %zu\n", strlen(decipherTextRecv));
	//printf("Server: Bytes Read of decipherText Message: %d\n", charsRead); //charsRead = bytes returned from recv
	//printf("Expected Length: %d Expected bytes: %zu\n", ntohl(decipherTextRecvSize), decipherTextExpectedSize);
	...
	cipherText = NULL;	//null cipherText
